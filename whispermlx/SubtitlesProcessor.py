...
from whispermlx.conjunctions import get_comma
from whispermlx.conjunctions import get_conjunctions
...
class SubtitlesProcessor:
    ...
    def process_segments(self, advanced_splitting=True):
        ...
        for i, segment in enumerate(self.segments):
            next_segment_start_time = (
                self.segments[i + 1]["start"] if i + 1 < len(self.segments) else None
            )

            if advanced_splitting:
                split_points = self.determine_advanced_split_points(
                    segment, next_segment_start_time
                )
                subtitles.extend(
                    self.generate_subtitles_from_split_points(
                        segment, split_points, next_segment_start_time
                    )
                )
            else:
                words = segment["words"]
    ...
    def determine_advanced_split_points(self, segment, next_segment_start_time=None):
        ...
        total_char_count = sum(
            len(word["word"]) if isinstance(word, dict) else len(word) + add_space for word in words
        )
    ...
    def generate_subtitles_from_split_points(self, segment, split_points, next_start_time=None):
        ...
        for split_point in split_points:
            ...
            if isinstance(fragment_words[0], dict):
                ...
                next_start_time_for_word = (
                    words[split_point + 1]["start"] if split_point + 1 < len(words) else None
                )
